...
-- This module is dedicated to [Trunc ⟨1⟩ (x == y)]

module experimental.PathGroupoid where

_=₁_ : ∀ {i} {A : Type i} → A → A → Type i
_=₁_ x y = Trunc ⟨1⟩ (x == y)

_=1_ : ∀ {i} {A : Type i} → A → A → Type i
_=1_ = _=₁_
...
coe₁ : ∀ {i} {A B : Type i} (_ : is-gpd B) (p : A =₁ B) → A → B
coe₁ B-level = Trunc-rec (→-level B-level) coe

transport₁ : ∀ {i j} {A : Type i} (B : A → Type j) {x y : A}
  (B-level : is-gpd (B y)) (p : x =₁ y) → (B x → B y)
